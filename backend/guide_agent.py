...
## 🔍 问题诊断（简要）
用1-2句话说明发现的问题（基于diagnosis）：
"{self.task.diagnosis}"

## 💡 熟悉程度探测（必须！）
**在深入之前，先问用户对这个项目的熟悉程度：**
...
**禁止在开场白中包含的内容：**
- ❌ 不要问"背景是什么"、"你负责什么"、"结果如何"等STAR问题
- ❌ 不要列出需要了解的信息清单
- ❌ 不要问优化目标相关的详细问题
- 这些问题应该在用户回答熟悉程度后，根据其选择再展开
...
示例：
\"\"\"
你好！我们来优化**{self.task.section}**这部分。

🔍 **我发现的问题：**
{self.task.diagnosis[:80]}...
...
### 2. 教育背景-课程/主修课程
- 格式：课程名称用顿号（、）或逗号（，）分隔，横向排列在同一行
- 不要使用列表格式（不要用 - 开头每行一个课程）
- 简洁直接，不添加描述性修饰语
